...
func newHandshake(cache AuthKeyStorager) *handshake {
	s := &handshake{
...
func (s *handshake) onHandshake(conn *net2.TcpConnection, payload *mtproto.ZProtoRawPayload) (*mtproto.ZProtoRawPayload, error) {
	...
	switch hmsg.MTPMessage.Object.(type) {
	case *mtproto.TLReqPq:
		res, err = s.onReqPq(state, hmsg.MTPMessage.Object.(*mtproto.TLReqPq))
		state.State = mtproto.STATE_pq_res
	case *mtproto.TLReq_DHParams:
		res, err = s.onReq_DHParams(hmsg.State, hmsg.MTPMessage.Object.(*mtproto.TLReq_DHParams))
		state.State = mtproto.STATE_DH_params_res
	case *mtproto.TLSetClient_DHParams:
		res, err = s.onSetClient_DHParams(hmsg.State, hmsg.MTPMessage.Object.(*mtproto.TLSetClient_DHParams))
	...
	if err != nil {
		state.ResState = mtproto.RES_STATE_ERROR
	} else {
		state.ResState = mtproto.RES_STATE_OK
	}
	...
	zmsg := &mtproto.ZProtoHandshakeMessage{
		State: state,
		MTPMessage: &mtproto.MTPRawMessage{
			AuthKeyId: 0,
			QuickAckId: 0,
			Payload: mtpMessage.Encode(),
		},
	}
	return &mtproto.ZProtoRawPayload{Payload: zmsg.Encode()}, nil
}

// req_pq#60469778 nonce:int128 = ResPQ;
func (s *handshake) onReqPq(state *mtproto.HandshakeState, request *mtproto.TLReqPq) (*mtproto.TLResPQ, error) {
	// state.State = mtproto.STATE_pq_res
	// state.ResState = mtproto.RES_STATE_OK
	authKeyMD := &mtproto.AuthKeyMetadata{}
	if err := proto.Unmarshal(state.Ctx, authKeyMD); err != nil {
		glog.Errorf("onReqPq - Unmarshal auth_key_metadata error: %v", err)

		return nil, err
	}

	glog.Infof("ReqPq - auth_key_metadata, request: %s", logger.JsonDebugData(authKeyMD), logger.JsonDebugData(request))
	...
	glog.Infof("process onReqPq - reply: %s", logger.JsonDebugData(resPQ))
	return resPQ, nil
}
...
func (s *handshake) onReq_DHParams(state *mtproto.HandshakeState, request *mtproto.TLReq_DHParams) (*mtproto.Server_DH_Params, error) {
	...
	glog.Infof("Req_DHParams - request: {%v}", request)
...
func (s *handshake) onSetClient_DHParams(state *mtproto.HandshakeState, request *mtproto.TLSetClient_DHParams) (*mtproto.SetClient_DHParamsAnswer, error) {
	...
	glog.Infof("SetClient_DHParams - request: %s", logger.JsonDebugData(request))
...
func (s *handshake) onMsgsAck(state *mtproto.HandshakeState, request *mtproto.TLMsgsAck) error {
	if state.ResState != mtproto.RES_STATE_OK {
		return fmt.Errorf("invalid state: %v", state)
	}

	switch state.State {
	case mtproto.STATE_pq_res:
		state.State = mtproto.STATE_pq_ack
	case mtproto.STATE_DH_params_res:
		state.State = mtproto.STATE_DH_params_ack
	case mtproto.STATE_dh_gen_res:
		state.State = mtproto.STATE_dh_gen_ack
	default:
		return fmt.Errorf("invalid state: %v", state)
	}
	return nil
